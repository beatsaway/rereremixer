...
// Layer mix removed in v1.02

// Synthesizer instances
let synthInstances = {};
let synthModeEnabled = false;
let humanizeTimingEnabled = false;
...
// Synthesizer DOM elements
const enableSynthMode = document.getElementById('enableSynthMode');
const synthSoundOptions = document.getElementById('synthSoundOptions');
const synthBass = document.getElementById('synthBass');
const synthKick = document.getElementById('synthKick');
const synthSnare = document.getElementById('synthSnare');
const synthHiHat = document.getElementById('synthHiHat');
const synthClap = document.getElementById('synthClap');
const humanizeTiming = document.getElementById('humanizeTiming');

// Preset dropdown elements
const bassPresetSelect = document.getElementById('bassPresetSelect');
const kickPresetSelect = document.getElementById('kickPresetSelect');
const snarePresetSelect = document.getElementById('snarePresetSelect');
const hihatPresetSelect = document.getElementById('hihatPresetSelect');
const clapPresetSelect = document.getElementById('clapPresetSelect');
const randomizePresetsButton = document.getElementById('randomizePresetsButton');
...
// Initialize synthesizer instances
function initializeSynthesizers() {
    try {
        ...
        // Populate preset dropdowns and apply default presets
        populatePresetDropdowns();
        applyDefaultPresets();
    } catch (error) {
        console.error('Error initializing synthesizers:', error);
    }
}

// Populate preset dropdowns from sound_presets.js
function populatePresetDropdowns() {
    if (typeof soundPresets === 'undefined') {
        console.warn('soundPresets not loaded');
        return;
    }
    
    // Populate bass dropdown
    if (bassPresetSelect && soundPresets.bass) {
        bassPresetSelect.innerHTML = '';
        soundPresets.bass.forEach((preset, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = preset.name;
            bassPresetSelect.appendChild(option);
        });
    }
    
    // Populate kick dropdown
    if (kickPresetSelect && soundPresets.kick) {
        kickPresetSelect.innerHTML = '';
        soundPresets.kick.forEach((preset, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = preset.name;
            kickPresetSelect.appendChild(option);
        });
    }
    
    // Populate snare dropdown
    if (snarePresetSelect && soundPresets.snare) {
        snarePresetSelect.innerHTML = '';
        soundPresets.snare.forEach((preset, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = preset.name;
            snarePresetSelect.appendChild(option);
        });
    }
    
    // Populate hihat dropdown
    if (hihatPresetSelect && soundPresets.hihat) {
        hihatPresetSelect.innerHTML = '';
        soundPresets.hihat.forEach((preset, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = preset.name;
            hihatPresetSelect.appendChild(option);
        });
    }
    
    // Populate clap dropdown
    if (clapPresetSelect && soundPresets.clap) {
        clapPresetSelect.innerHTML = '';
        soundPresets.clap.forEach((preset, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = preset.name;
            clapPresetSelect.appendChild(option);
        });
    }
}

// Apply default presets to synthesizers
function applyDefaultPresets() {
    if (typeof soundPresets === 'undefined') return;
    
    // Apply default bass preset (Solar Flare - index 3)
    if (synthInstances.bass && soundPresets.bass && soundPresets.bass[3]) {
        synthInstances.bass.params = { ...soundPresets.bass[3].params };
        if (bassPresetSelect) bassPresetSelect.value = 3;
    }
    
    // Apply default kick preset (Cosmic Impact - index 3)
    if (synthInstances.kick && soundPresets.kick && soundPresets.kick[3]) {
        synthInstances.kick.params = { ...soundPresets.kick[3].params };
        if (kickPresetSelect) kickPresetSelect.value = 3;
    }
    
    // Apply default snare preset (Echo Chamber - index 1)
    if (synthInstances.snare && soundPresets.snare && soundPresets.snare[1]) {
        synthInstances.snare.params = { ...soundPresets.snare[1].params };
        if (snarePresetSelect) snarePresetSelect.value = 1;
    }
    
    // Apply default hihat preset (Quantum Spark - index 0)
    if (synthInstances.hihat && soundPresets.hihat && soundPresets.hihat[0]) {
        synthInstances.hihat.params = { ...soundPresets.hihat[0].params };
        if (hihatPresetSelect) hihatPresetSelect.value = 0;
    }
    
    // Apply default clap preset (Galactic Snap - index 4)
    if (synthInstances.clap && soundPresets.clap && soundPresets.clap[4]) {
        synthInstances.clap.params = { ...soundPresets.clap[4].params };
        if (clapPresetSelect) clapPresetSelect.value = 4;
    }
}

// Apply preset to specific synthesizer
function applyPreset(synthType, presetIndex) {
    if (typeof soundPresets === 'undefined' || !synthInstances[synthType]) return;
    
    const presets = soundPresets[synthType];
    if (presets && presets[presetIndex]) {
        synthInstances[synthType].params = { ...presets[presetIndex].params };
        console.log(`Applied ${presets[presetIndex].name} preset to ${synthType}`);
    }
}

// Get enabled synthesized sounds
function getEnabledSynthSounds() {
    const enabled = [];
    if (synthBass && synthBass.checked) enabled.push('bass');
    if (synthKick && synthKick.checked) enabled.push('kick');
    if (synthSnare && synthSnare.checked) enabled.push('snare');
    if (synthHiHat && synthHiHat.checked) enabled.push('hihat');
    if (synthClap && synthClap.checked) enabled.push('clap');
    return enabled;
}

// Play random synthesized sound
function playRandomSynthSound() {
    const enabledSounds = getEnabledSynthSounds();
    if (enabledSounds.length === 0) return;
    
    const randomSound = enabledSounds[Math.floor(Math.random() * enabledSounds.length)];
    const synthInstance = synthInstances[randomSound];
    
    if (synthInstance) {
        try {
            synthInstance.play();
        } catch (error) {
            console.error(`Error playing ${randomSound} sound:`, error);
        }
    }
}
...
function calculateDuration(noteValue, bpm) {
    ...
    switch (noteValue) {
        ...
        case '1/64T': return (beatDuration / 16) * (4/3); // extended note
        case '1/32': return beatDuration / 8;
        case '1/32T': return (beatDuration / 8) * (4/3); // extended note
...
// Play a random slice of the audio or synthesized sound
function playRandomSlice() {
    ...
    // Check if synthesizer mode is enabled
    if (synthModeEnabled) {
        playRandomSynthSound();
        
        // Update status
        statusDiv.textContent = `Remixing (Synth Mode)... ${remixSequence[currentRemixIndex].noteValue} (${currentRemixIndex + 1}/128)`;
    ...
    // Original audio slice functionality
...
function startRemix() {
    ...
    // Allow synth mode without audio buffer
    if (!synthModeEnabled && !audioBuffer) return;
...
function scheduleAllNotes() {
    ...
    for (let i = 0; i < remixSequence.length; i++) {
        ...
        let noteDuration = note.durationMs / 1000; // Convert to seconds
        if (humanizeTimingEnabled) {
            const durJitter = (Math.random() * 0.1 - 0.05) * noteDuration; // ±5%
            noteDuration = Math.max(0, noteDuration + durJitter);
        }
        const delayTime = parseInt(delaySlider.value) / 1000; // Get delay time
        
        // Apply humanize jitter to the start time (±5% of noteDuration)
        const jitter = humanizeTimingEnabled ? (Math.random() * 0.1 - 0.05) * noteDuration : 0; // seconds
        const scheduledAt = currentTime + jitter;

        // Schedule this note to play at scheduledAt
        ...
        }, Math.max(0, (scheduledAt - now) * 1000));
...
function updateDurationValues() {
    const bpm = parseInt(bpmSlider.value);
    const noteTypes = ['1/64', '1/64T', '1/32', '1/32T', '1/16', '1/16T', '1/12', '1/12T', '1/8', '1/8T', '1/4', '1/4T', '1/2', '1/2T'];
...
function updateActivePreset(presetName) {
    ...
    const presetButtons = document.querySelectorAll('.reset-btn, .chop-btn, .shuffle-btn, .swing-btn, .break-btn, .dub-btn, .slow-btn, .microburst-btn, .trap-btn, .triplet-town-btn, .halftime-btn, .poly32-btn, .dotted-btn, .skank-btn, .breakcore-btn, .techno-btn, .ambient-btn');
...
// Helper: set multiple probabilities by note
function setProbabilities(map) {
    const sliders = document.querySelectorAll('.probability-slider input[type="range"]');
    sliders.forEach(slider => {
        const note = slider.dataset.note;
        slider.value = map[note] ?? 0;
        const label = slider.parentElement.querySelector('label');
        const probValue = label.querySelector('.prob-value');
        probValue.textContent = slider.value;
    });
...
// Microburst (fast rolls)
document.getElementById('microburstButton').addEventListener('click', () => {
    setProbabilities({ '1/64': 40, '1/64T': 20, '1/32': 20, '1/32T': 10, '1/16': 10 });
    updateActivePreset('microburst');
});

// Trap Roll
document.getElementById('trapButton').addEventListener('click', () => {
    setProbabilities({ '1/32': 25, '1/32T': 25, '1/16': 20, '1/8': 15, '1/64': 15 });
    updateActivePreset('trap');
});

// Triplet Town
document.getElementById('tripletTownButton').addEventListener('click', () => {
    setProbabilities({ '1/16T': 35, '1/8T': 35, '1/4T': 20, '1/12': 10 });
    updateActivePreset('tripletTown');
});

// Half-Time Groove
document.getElementById('halftimeButton').addEventListener('click', () => {
    setProbabilities({ '1/2': 50, '1/4': 30, '1/8': 20 });
    updateActivePreset('halftime');
});

// Polyrhythm 3:2 feel
document.getElementById('poly32Button').addEventListener('click', () => {
    setProbabilities({ '1/12': 30, '1/8': 30, '1/16T': 20, '1/4': 20 });
    updateActivePreset('poly32');
});

// Dotted Feel (longer “T”)
document.getElementById('dottedButton').addEventListener('click', () => {
    setProbabilities({ '1/8T': 30, '1/4T': 30, '1/16T': 25, '1/2T': 15 });
    updateActivePreset('dotted');
});

// Offbeat Skank
document.getElementById('skankButton').addEventListener('click', () => {
    setProbabilities({ '1/4': 40, '1/8': 40, '1/16': 10, '1/8T': 10 });
    updateActivePreset('skank');
});

// Breakcore Burst
document.getElementById('breakcoreButton').addEventListener('click', () => {
    setProbabilities({ '1/64': 35, '1/32': 35, '1/16': 20, '1/8': 10 });
    updateActivePreset('breakcore');
});

// Techno Drive
document.getElementById('technoButton').addEventListener('click', () => {
    setProbabilities({ '1/16': 50, '1/8': 30, '1/32': 10, '1/4': 10 });
    updateActivePreset('techno');
});

// Ambient Wash
document.getElementById('ambientButton').addEventListener('click', () => {
    setProbabilities({ '1/2': 40, '1/4T': 30, '1/8T': 20, '1/4': 10 });
    updateActivePreset('ambient');
});
...
// Synthesizer mode event listeners
enableSynthMode.addEventListener('change', (e) => {
    synthModeEnabled = e.target.checked;
    if (synthSoundOptions) {
        synthSoundOptions.style.display = synthModeEnabled ? 'block' : 'none';
    }
    
    // Initialize synthesizers if enabling synth mode
    if (synthModeEnabled) {
        initializeAudioContext();
    }
    ...
    // Update status
    if (synthModeEnabled) {
        statusDiv.textContent = 'Synthesizer mode enabled! Click Start Remix to begin.';
        playBtn.disabled = false; // Enable play button in synth mode
        holdBtn.disabled = false; // Enable hold button in synth mode
        if (randomizePresetsButton) randomizePresetsButton.style.display = 'inline-block';
    } else {
        statusDiv.textContent = audioBuffer ? 'Audio loaded! Click Play to hear it.' : 'Ready to record or upload audio';
        playBtn.disabled = !audioBuffer; // Disable if no audio in sample mode
        holdBtn.disabled = !audioBuffer;
        if (randomizePresetsButton) randomizePresetsButton.style.display = 'none';
    }
});

// Humanize timing checkbox
if (humanizeTiming) {
    humanizeTiming.addEventListener('change', (e) => {
        humanizeTimingEnabled = e.target.checked;
        // If remixing, reschedule with new jitter state
        if (isRemixing) {
            updateRemixSequence();
        }
    });
...
// Preset dropdown event listeners
if (bassPresetSelect) {
    bassPresetSelect.addEventListener('change', (e) => {
        applyPreset('bass', parseInt(e.target.value));
    });
}

if (kickPresetSelect) {
    kickPresetSelect.addEventListener('change', (e) => {
        applyPreset('kick', parseInt(e.target.value));
    });
}

if (snarePresetSelect) {
    snarePresetSelect.addEventListener('change', (e) => {
        applyPreset('snare', parseInt(e.target.value));
    });
}

if (hihatPresetSelect) {
    hihatPresetSelect.addEventListener('change', (e) => {
        applyPreset('hihat', parseInt(e.target.value));
    });
}

if (clapPresetSelect) {
    clapPresetSelect.addEventListener('change', (e) => {
        applyPreset('clap', parseInt(e.target.value));
    });
}

// Randomize presets button
if (randomizePresetsButton) {
    randomizePresetsButton.addEventListener('click', () => {
        if (typeof soundPresets === 'undefined') return;
        
        const types = ['bass', 'kick', 'snare', 'hihat', 'clap'];
        types.forEach(type => {
            const list = soundPresets[type];
            if (list && list.length > 0) {
                const idx = Math.floor(Math.random() * list.length);
                applyPreset(type, idx);
                // Sync dropdown UI
                const selectMap = {
                    bass: bassPresetSelect,
                    kick: kickPresetSelect,
                    snare: snarePresetSelect,
                    hihat: hihatPresetSelect,
                    clap: clapPresetSelect
                };
